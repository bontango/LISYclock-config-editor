...
#   - .env Datei im Projektverzeichnis oder in ../LISYclock/ (siehe .env.example)
#
# Verwendung:
#   .\convert_and_deploy.ps1
...
if (Test-Path ".env") {
    $envFile = ".env"
} elseif (Test-Path "..\LISYclock\.env") {
    $envFile = "..\LISYclock\.env"
}
if (-not $envFile) {
    Err ".env nicht gefunden (weder im aktuellen Verzeichnis noch in ..\LISYclock\)."
...
$conversions = @(
    @{ Src = "Bedienungsanleitung.md"; Dst = "Bedienungsanleitung.html" },
    @{ Src = "user_manual.md";         Dst = "user_manual.html" },
    @{ Src = "manuale_duso.md";        Dst = "manuale_duso.html" }
)
